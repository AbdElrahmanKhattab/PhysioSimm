...
const initDbPromise = new Promise((resolve, reject) => {
  ...
  conn.query(`CREATE DATABASE IF NOT EXISTS \`${dbConfig.database}\``, (err) => {
    if (err) {
      ...
      // Don't reject here, as the DB might already exist or user might not have create permission
      // but we want to try connecting to it anyway.
      // However, if we can't connect at all, the next step will fail.
      // For Railway, usually the DB exists.
...
async function init() {
  try {
    ...
    // Do not exit process, let it try to recover or just log error
    // process.exit(1); 
  }
}
