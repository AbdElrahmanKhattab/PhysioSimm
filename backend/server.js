...
const dbConfig = {
  ...
  password: process.env.MYSQLPASSWORD || 'Mazen198165967#',
  port: process.env.MYSQLPORT || 3306,
  database: process.env.MYSQLDATABASE || 'PhysioCaseLab',
...
// Initial connection to create DB if not exists (only if not using a managed DB like Railway)
const initDbPromise = new Promise((resolve, reject) => {
  if (process.env.MYSQLDATABASE) {
    console.log(`Using existing database: ${process.env.MYSQLDATABASE}`);
    resolve();
    return;
  }
...
app.get('/api/admin/overview', authMiddleware('admin'), async (req, res) => {
  try {
    ...
    const [caseCount] = await queryAsync('SELECT COUNT(*) as count FROM cases');
    const [completionCount] = await queryAsync('SELECT COUNT(*) as count FROM progress WHERE isCompleted = 1');
    const [premiumCount] = await queryAsync("SELECT COUNT(*) as count FROM users WHERE membershipType = 'premium'");

    // Recent Activity: Combine new users and new cases
    const recentUsers = await queryAsync(`
      SELECT 'user_joined' as type, email as title, createdAt as date 
      FROM users ORDER BY createdAt DESC LIMIT 5
    `);

    const recentCases = await queryAsync(`
      SELECT 'case_created' as type, title, createdAt as date 
      FROM cases ORDER BY createdAt DESC LIMIT 5
    `);

    // Combine and sort by date
    const activity = [...recentUsers, ...recentCases]
