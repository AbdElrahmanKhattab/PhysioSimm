...
function CaseRunnerPage({ auth }) {
  ...
  const handleAnswer = async (optionId) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/cases/${caseData.id}/steps/${currentStep.id}/answer`,
        {
          method: 'POST',
          headers: {
  ...
  const canGoNext =
    currentStep?.type !== 'mcq' || (feedback === null && selectedOption !== null)
...
function McqStep({ step, selectedOption, feedback, onAnswer }) {
