...
export default function CaseEditorPage({ auth }) {
        ...
        if (isEdit) {
            const load = async () => {
                try {
                    ...
                    // Load Steps
                    const stepsRes = await fetch(`${API_URL}/api/admin/cases/${id}/steps`, {
                        headers: { Authorization: `Bearer ${auth.token}` }
                    })
    ...
    const handleAddStep = async (type) => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/admin/cases/${id}/steps`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${auth.token}`,
    ...
    const handleDeleteStep = async (stepId) => {
        ...
        try {
            await fetch(`${API_URL}/api/admin/steps/${stepId}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${auth.token}` }
            })
                ...
                {activeTab === 'steps' && (
                    <div className="steps-section">
                            ...
                            {steps.map((step, idx) => (
                                <div key={step.id} className="step-card">
                                    ...
                                    <div className="step-preview">
                                        {/* Simplified preview */}
                                        {step.type === 'info' && <p>Info Step</p>}
                                        {step.type === 'mcq' && <p>Question: {step.question}</p>}
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <p style={{ marginTop: '1rem', color: '#6b7280', fontSize: '0.9rem' }}>
                            * Note: Full step content editing is coming in the next update. Steps are created with default templates.
                        </p>
                    </div>
                )}
            </div>
